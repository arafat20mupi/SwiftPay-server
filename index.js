...
const transactionsRequestedCollection = client
    .db('SwiftPay')
    .collection('transactionsRequested')
...
async function run() {
    try {
        ...
        app.get('/requested/agents', async (req, res) => {
            try {
                const agents = await requestedCollection
                    .find({ role: 'agent' })
                    .toArray()
        ...
        app.post('/api/sendMoney', async (req, res) => {
            ...
            try {
                ...
                if (senderUser.balance < amountNum) {
                    // Check balance
        ...
        app.put('/api/approveCashIn/:id', async (req, res) => {
            ...
            try {
                // Find the request
                const request = await requestedCollection.findOne({
                ...
                if (agent.balance < amount) {
                    return res
                        .status(400)
                        .json({ message: 'Insufficient balance in agent account' })
                }
